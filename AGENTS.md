...
**M2 Schritt 1 — Quellen-RAG-Gerüst (Branch `m2/rag-geruest`):** RAG-Governance und Ingestion-Infrastruktur stehen. EINE Qdrant-Collection `ua_lsa_chunks` mit Pflicht-Payload-Filtern (trust_level, subject, confession_context) statt separater Collections pro Fach/Konfession (ADR 0010). Local-first Embedding via Ollama (`qwen3-embedding:4b`). Textextraktion Schritt 1: `text/plain`, `text/html`, `application/pdf` (via `pdf-parse`); OCR-Scan-PDFs ausgenommen → M2.4. Konfessionstrennung erzwungen serverseitig durch Retrieval-Filter (kein Cross-Strang-Retrieval). Noch nicht real: OCR-Worker, vollständige Ingestion-Pipeline, LLM-Provider-Integration, Retrieval-UI, Zitier-Assemblierung.
