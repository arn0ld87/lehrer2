...
**M0 (Foundations & Governance) abgeschlossen; M1 (UI-Shell + Datenmodell/Export) gemergt; M2 in Umsetzung — Kern auf `main` (Stand 2026-06-26).** Repo-Gerüst, Doku, lauffähige Next.js-UI-Shell, persistente Datenschicht und die RAG-/LLM-/Generierungs-Pipeline stehen auf `main`. Committet: `README.md`, `PLAN.md`, `CLAUDE.md`, `AGENTS.md`, ein weitgehend vollständiger `docs/`-Baum, `unterrichtsassistenz-lsa-design-kit/`, Tooling/CI (`package.json`, `compose.yaml`, `.env.example`, `.editorconfig`, `.nvmrc`, `.prettierrc.json`, `.prettierignore`), `scripts/` (`verify-docs.sh`, `verify-docs.mjs`, `check-schema-drift.mjs`), `data/source-registry.seed.yaml` und `.github/` (CI-Workflow, Issue-Templates, PR-Template).
...
**M2 — Quellen-RAG + Generierung (Kern auf `main`):** RAG-Governance, Ingestion und der quellengebundene Generierungs-Slice stehen. EINE Qdrant-Collection `ua_lsa_chunks` mit Pflicht-Payload-Filtern (trust_level, subject, confession_context) statt separater Collections pro Fach/Konfession (ADR 0010). Local-first Embedding via Ollama (`qwen3-embedding:4b`, committeter Default in `.env.example`; OpenAI-kompatibler Pfad konfigurierbar). Textextraktion: `text/plain`, `text/html`, `application/pdf` (via `pdf-parse`); OCR-Worker unter `src/lib/rag/ocr/` (Tesseract Default, jpeg/png + Scan-PDF in Arbeit). RAG-Pipeline `src/lib/rag/` (`extract`/`chunk`/Embedding/`qdrant`/`retrieve`/`citation`/`eval`/`deletion` über Repository). Provider-agnostische LLM-Schicht `src/lib/llm/` (`ollama-chat` Default, `cloud` gegatet, `fake`, `provider`) mit **fail-closed PII-Gate** (`redaction.ts`/`guard.ts`/`gate.ts`). Generierung `src/lib/generation/` + `src/app/actions/` für `/planung` + `/arbeitsblaetter`: Retrieval → LLM → DB-Insert, Offline-Degradation via `unavailable`. Konfessionstrennung serverseitig durch Retrieval-Filter erzwungen (kein Cross-Strang-Retrieval). Noch nicht real: Korrekturassistenz (`/korrektur` Mock, M3), Dashboard (Mock), produktive Cloud-Grants, Job-Orchestrierung (Redis/BullMQ), vollständiger Scan-PDF-Pfad.
...
| Entscheidungen (Modular Monolith, LLM-Schicht, Source-Governance, Local-first, Drizzle, Curriculum, Auth, Export, Pseudonym-Retention) | `docs/adr/0001`–`0010` (0005/0007/0008 Akzeptiert 2026-06-22; 0010 RAG-Ingestion via Qdrant)                               |
...
| Produktvision, MVP, Akzeptanz, User Flows                                                                                              | `docs/product/PRODUCT_VISION.md`, `MVP_SCOPE.md`, `ACCEPTANCE_CRITERIA.md`, `USER_FLOWS.md`, `WORKSHEET_GENERATOR_SPEC.md` |
| CI/CD, Entwicklung, Backup & Restore                                                                                                   | `docs/operations/CI_CD.md`, `DEVELOPMENT.md`, `BACKUP_AND_RECOVERY.md`, `RECOVERY_TEST_PLAN.md`, `GITHUB_SETUP.md`         |
