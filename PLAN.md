...
> **Status: M2 in Umsetzung (Stand 2026-06-26).** M0 (Governance) abgeschlossen, M1 (UI-Shell + Datenmodell/Export) gemergt. Auf `main` läuft eine lauffähige Anwendung mit echter Datenschicht (PostgreSQL/Drizzle), RAG-Gerüst (Qdrant), Ingestion, Retrieval/Reranking/Zitation, provider-agnostischer LLM-Schicht mit fail-closed PII-Gate und einem vertikalen Slice `/planung` + `/arbeitsblaetter`, der real generiert (Retrieval → LLM → DB, mit Offline-Degradation). **Noch nicht** überprüft produktiv: Korrekturassistenz (`/korrektur`, M3 — nur Spec), Cloud-LLM-Freigaben, vollständige Job-Orchestrierung. Jede Datenschutz-/RAG-/Lehrplankonformität wird erst behauptet, wenn sie überprüft umgesetzt ist.
...
- **M0 – Foundations & Governance** _(abgeschlossen, Stand 2026-06-22)_: Vision/Scope, ADRs (0001–0010; 0005/0007/0008 Akzeptiert), Datenschutz-/Löschkonzept, Threat Model, Quellenpolicy, initiale Quellenregistry, Curriculummodell, Dev-/CI-Grundgerüst (Paketmanager auf pnpm vereinheitlicht), UX-Flows, alle sechs offenen Grundsatzentscheidungen getroffen.
- **M1 – Unterrichtsplanung MVP** _(gemergt)_: Datenmodell Artefakte, Export-Architektur, Planungs-/Arbeitsblatt-Spezifikation. Schritt 1 (UI-Shell) und Schritt 2 (Branch `m1/data-model-export`: Drizzle-Schema Auth/Tenant/Curriculum/Artefakte/Provenienz, Migrationen, Postgres-Repositories + Mapping, Export-Subsystem DOCX/PDF, CI-Gates Schema-Drift + Migration-Review-Flag) umgesetzt. Die Planungs-/Arbeitsblatt-Generierungslogik selbst wurde im M2-Slice nachgezogen (hing an RAG).
- **M2 – Quellen-RAG mit Nachweisen** _(in Umsetzung, Kern auf `main`)_: Umgesetzt — RAG-Gerüst (eine Qdrant-Collection `ua_lsa_chunks` mit Pflicht-Payload-Filtern, ADR 0010), Ingestion (Extraktion `text/plain`/`html`/`pdf`, Chunking, Embedding, Indexierung), Retrieval/Reranking/Zitation, provider-agnostische LLM-Schicht (Ollama Default, Cloud gegatet, fake) mit **fail-closed PII-Gate** (Redaction/Guard/Gate), vertikaler Slice `/planung` + `/arbeitsblaetter` mit echter Generierung (Retrieval → LLM → DB-Insert, Offline-Degradation), `/quelle` mit echten RAG-Kennzahlen, Golden-Questions-Eval-Suite + Drift-Erkennung, Quellenwiderruf/Re-Ingestion/kaskadierende Löschung, OCR-Worker (Tesseract Default; jpeg/png + pdf-parse). Ausstehend — Scan-PDF-Pfad vollständig, Job-Orchestrierung (Redis/BullMQ), Retrieval-UI-Ausbau.
- **M3 – Korrekturassistenz MVP** _(Spezifikation, Implementierung ausstehend)_: Korrekturworkflow spezifiziert; `/korrektur` läuft derzeit gegen Mock-Daten. Noch umzusetzen: Pseudonymisierung/Redaction für Schülerarbeiten, Rubric-Scoring, Feedbackformat, Upload-/OCR-Sicherheit (`SENSITIVE_STUDENT`).
...
ADRs: [0001](docs/adr/0001-modular-monolith-first.md) · [0002](docs/adr/0002-provider-agnostic-llm-layer.md) · [0003](docs/adr/0003-source-governance-before-ingestion.md) · [0004](docs/adr/0004-local-first-student-data.md) · [0005](docs/adr/0005-orm-drizzle.md) · [0006](docs/adr/0006-curriculum-modeling.md) · [0007](docs/adr/0007-auth-solution.md) _(Akzeptiert)_ · [0008](docs/adr/0008-document-export-stack.md) _(Akzeptiert)_ · [0009](docs/adr/0009-pseudonym-retention.md) · [0010](docs/adr/0010-rag-ingestion-qdrant.md) _(RAG-Ingestion via Qdrant)_. Offene Entscheidungen: alle 6 aus [OPEN_QUESTIONS.md](docs/decisions/OPEN_QUESTIONS.md) entschieden (Stand 2026-06-22). GitHub-Planung & Status: [GITHUB_SETUP.md](docs/operations/GITHUB_SETUP.md).
